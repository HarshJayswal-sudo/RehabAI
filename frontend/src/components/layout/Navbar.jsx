...
import { User, Home, LayoutDashboard, Activity, Settings, LogOut, Crown } from 'lucide-react';
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer' }} onClick={() => navigateTo('landing')}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '42px', height: '42px', backgroundColor: '#FFF', borderRadius: '50%', boxShadow: '0 4px 10px rgba(0,0,0,0.05)' }}>
             <img src={iconImg} alt="Lotus" style={{ height: '24px' }} />
          </div>
        ...
        <nav style={{ height: '100%', display: 'flex', alignItems: 'center' }}>
          <ul style={{ 
            listStyle: 'none', display: 'flex', margin: 0, padding: 0, gap: '30px', height: '100%', alignItems: 'center'
          }}>
            {[
              ...
              { id: 'session', label: 'Live AI Session', icon: <Activity size={16} strokeWidth={2.5} /> },
            ].map(item => {
        ...
        {/* Actions & Minimal Profile */}
        <div style={{ display: 'flex', gap: '25px', alignItems: 'center' }}>
          <button className="btn btn-primary" style={{ padding: '12px 28px', fontSize: '13px', fontWeight: 700, borderRadius: '50px', boxShadow: '0 8px 20px rgba(100,114,217,0.25)', letterSpacing: '0.5px' }} onClick={() => navigateTo('session')}>
            START SESSION
          </button>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <AnimatePresence>
              {isProfileOpen && (
                ...
                >
                  <div style={{ padding: '25px', borderBottom: '1px solid #F1F5F9', display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <div style={{ width: '50px', height: '50px', borderRadius: '50%', backgroundColor: 'var(--accent-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent-color)' }}>
                      <User size={24} />
                    </div>
                    <div>
                      <h4 style={{ margin: 0, fontSize: '18px', fontWeight: 800, color: '#111' }}>John Doe</h4>
                      <p style={{ margin: 0, fontSize: '13px', color: '#64748B', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                        <Crown size={14} color="#F59E0B" /> Pro Member
                      </p>
                    </div>
                  </div>
                  <div style={{ padding: '10px' }}>
                    <button style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 15px', background: 'transparent', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '14px', fontWeight: 600, color: '#475569', transition: 'all 0.2s' }} onMouseOver={(e) => {e.currentTarget.style.background = '#F8FAFC'; e.currentTarget.style.color = 'var(--accent-color)'}} onMouseOut={(e) => {e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#475569'}} onClick={() => setIsProfileOpen(false)}>
                  ...
                  <div style={{ padding: '10px', borderTop: '1px solid #F1F5F9' }}>
                    <button style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 15px', background: 'transparent', border: 'none', borderRadius: '12px', cursor: 'pointer', fontSize: '14px', fontWeight: 600, color: '#EF4444', transition: 'all 0.2s' }} onMouseOver={(e) => {e.currentTarget.style.background = '#FEF2F2'}} onMouseOut={(e) => {e.currentTarget.style.background = 'transparent'}} onClick={() => setIsProfileOpen(false)}>
